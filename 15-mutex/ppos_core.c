...
// DESCOMENTAR APENAS UM ALGORITMO DE ESCALONAMENTO
// #define __SCHEDULER_IN_FCS__ scheduler_in_fcs()
#define __SCHEDULER_IN_ENVELHECIMENTO__ scheduler_in_envelhecimento()

#ifdef __SCHEDULER_IN_ENVELHECIMENTO__
#define __FATOR_ENVELHECIMENTO__ 1
#endif

// define o algoritmo selecionado
#ifdef __SCHEDULER_IN_FCS__
// Define o tipo de algoritmo de escalonaento que sera executado
#define __PPOS_SCHEDULER_ALGORITHM__() __SCHEDULER_IN_FCS__
#endif

#ifdef __SCHEDULER_IN_ENVELHECIMENTO__
#define __PPOS_SCHEDULER_ALGORITHM__() __SCHEDULER_IN_ENVELHECIMENTO__
#endif
...
// Define se a retirada das terfas dormindo será feita por meio de uma tarefa, 
// a qual será escalonada, ou se esta retirada fará parte do dispather, o qual
// varrerá a lista de tarefas dormindo toda vez que for executado
//
// 0 -> Controle das tarefas dormindo faz parte do dispatcher
// 1 -> Controle das tarefas dormindo é uma tarefa de sistema escalonada
#define __PPOS_CONTROLE_TAREFAS_DORMINDO_EH_UMA_TAREFA_DE_SISTEMA 0
...
typedef enum {
    PPOS_CORE_ID_SYSTEM_TASK_MAIN = 0,                  // NAO MEXER NA POSICAO
    PPOS_CORE_ID_SYSTEM_TASK_DISPATCHER,                // NAO MEXER NA POSICAO
    #if __PPOS_CONTROLE_TAREFAS_DORMINDO_EH_UMA_TAREFA_DE_SISTEMA == 1
    PPOS_CORE_ID_SYSTEM_TASK_CONTROL_SLEEP_TASKS,       // NAO MEXER NA POSICAO
    #endif
    // adicione mais caso precise

    
    PPOS_CORE_ID_SYSTEM_TASK_LENGTH                     // NAO MEXER NA POSICAO
} pppos_core_id_system_tasks_enum_t;
...
// TCB's das tarefas de sistema
static task_t ppos_core_system_tasks[PPOS_CORE_ID_SYSTEM_TASK_LENGTH];
...
// array de ponteiros de funcao das tarefas do sistema
static func_t ppos_core_system_functions[PPOS_CORE_ID_SYSTEM_TASK_LENGTH] = {
    [PPOS_CORE_ID_SYSTEM_TASK_DISPATCHER] = &dispatcher,
    #if __PPOS_CONTROLE_TAREFAS_DORMINDO_EH_UMA_TAREFA_DE_SISTEMA == 1
    [PPOS_CORE_ID_SYSTEM_TASK_CONTROL_SLEEP_TASKS] = &control_sleep_tasks,
    #endif
    [PPOS_CORE_ID_SYSTEM_TASK_MAIN] = NULL,
};

//----------------------------------------------------------------------------------------------------
// private macro functions

#define __PPOS_SWITCH_CONTEXTO_TO_DISPATCHER__() ppos_core_system_tasks[PPOS_CORE_ID_SYSTEM_TASK_DISPATCHER].activations++; task_switch(&ppos_core_system_tasks[PPOS_CORE_ID_SYSTEM_TASK_DISPATCHER])
...
void ppos_init ()
{
    ...
    // Instala as tarefas de sistema
    for(i = 0; i < PPOS_CORE_ID_SYSTEM_TASK_LENGTH; i++) 
    {
        task_init(&ppos_core_system_tasks[i], ppos_core_system_functions[i], NULL);
        if(i != PPOS_CORE_ID_SYSTEM_TASK_MAIN) ppos_core_system_tasks[i].is_user_task = false;
    }
    ...
    ppos_core_task_current = &ppos_core_system_tasks[PPOS_CORE_ID_SYSTEM_TASK_MAIN];
...
void task_exit (int exit_code)
{
    ...
    while (ppos_core_task_current->queue_task_suspend != NULL) task_awake(ppos_core_task_current->queue_task_suspend, &ppos_core_task_current->queue_task_suspend);
    ...
    {
        free(ppos_core_task_current->context.uc_stack.ss_sp);
...
void task_sleep (int t) 
{
    ...
    //
    #if __PPOS_CONTROLE_TAREFAS_DORMINDO_EH_UMA_TAREFA_DE_SISTEMA == 1
    task_append_queue_ready(&ppos_core_system_tasks[PPOS_CORE_ID_SYSTEM_TASK_CONTROL_SLEEP_TASKS]);
    #endif
...
int sem_down (semaphore_t *s) 
{
    ...
    if(s->is_destroyed)
    {
        __PPOS_CORE_STD_MSG_ERRO__("Semaforo was destroyed");
        return -1;
    }
...
int mutex_init (mutex_t *m)
{
    if(m == NULL)
    {
        __PPOS_CORE_STD_MSG_ERRO__("Mutex NULL");
        return -1;
    }

    m->is_destroyed = false;

    // Mutex possui apenas uma chave, ou seja, apenas uma terefa obtem acesso ao recurso
    m->count = 1;

    // fila de tarefas suspensas vazia
    m->queue = NULL;
...
int mutex_lock (mutex_t *m)
{
    ...
    // O mutex foi destruído? se sim então
    if( m->is_destroyed )
    {
        __PPOS_CORE_STD_MSG_ERRO__("Mutex was destroyed");
        return -1;
    }

    __PPOS_DISABLE_PREEMP();

    // Alguma task já está usando esse mutex? se sim então
    if( --m->count < 0 )
    {
        // suspende a tarefa atual
        task_suspend(&m->queue);
...
int mutex_unlock (mutex_t *m)
{
    ...
    if( ++m->count <= 0 )
    {
        ...
        // remove a primeira tarefa da fila
        queue_remove((queue_t **) &m->queue, (queue_t *) m->queue);

        // adiciona a tarefa removida da fila do mutex na fila de tarefas prontas
        task_append_queue_ready(task_aux);
    }
...
int mutex_destroy (mutex_t *m) 
{
    ...
    // acorda todas as tarefas suspensas pelo mutex
    while(m->queue != NULL) { task_awake(m->queue, &m->queue); }
...
int barrier_wait (barrier_t *b)
{
    ...
    if(--b->count_to_awake > 0)
    {
        ...
        task_switch(&ppos_core_system_tasks[PPOS_CORE_ID_SYSTEM_TASK_DISPATCHER]);
    }
...
int mqueue_init (mqueue_t *queue, int max, int size)
{
    ...
    queue->buffer_circular = (void*) malloc(size * max);

    __PPOS_ENABLE_PREEMP();

    if(queue->buffer_circular == NULL)
        return -1;
...
int mqueue_send (mqueue_t *queue, void *msg)
{
    ...
    if ( sem_down(&queue->s_vaga) == -1 || sem_down(&queue->s_buffer) == -1) 
        return -1;

    __PPOS_DISABLE_PREEMP();

    memcpy( queue->buffer_circular + (queue->i_add*queue->msg_size), msg, queue->msg_size );

    __PPOS_ENABLE_PREEMP();

    if(++queue->i_add == queue->max_msg)
        queue->i_add = 0;

    if ( sem_up(&queue->s_buffer) == -1 || sem_up(&queue->s_item) == -1 ) 
        return -1;
...
int mqueue_recv (mqueue_t *queue, void *msg)
{
    ...
    if ( sem_down(&queue->s_item) == -1 ||  sem_down(&queue->s_buffer)) 
        return -1;

    __PPOS_DISABLE_PREEMP();

    memcpy(msg, queue->buffer_circular + (queue->i_rem*queue->msg_size), queue->msg_size);

    __PPOS_ENABLE_PREEMP();

    if(++queue->i_rem == queue->max_msg)
        queue->i_rem = 0;

    if ( sem_up(&queue->s_buffer) == -1 || sem_up(&queue->s_vaga) == -1) 
        return -1;
...
int mqueue_msgs (mqueue_t *queue)
{
    ...
    return abs(queue->i_add - queue->i_rem);
}
...
static void dispatcher(void * arg)
{
    ...
    task_remove_queue_ready(&ppos_core_system_tasks[PPOS_CORE_ID_SYSTEM_TASK_DISPATCHER]);

    //
    #if __PPOS_CONTROLE_TAREFAS_DORMINDO_EH_UMA_TAREFA_DE_SISTEMA == 1
    task_remove_queue_ready(&ppos_core_system_tasks[PPOS_CORE_ID_SYSTEM_TASK_CONTROL_SLEEP_TASKS]);
    #endif
    ...
    #else
    while( ppos_size_tasks_ready || ppos_size_tasks_sleep )
    #endif
    {
        #ifdef DEBUG_2
        ...
        #if __PPOS_CONTROLE_TAREFAS_DORMINDO_EH_UMA_TAREFA_DE_SISTEMA == 0
        {
            task_t *task_aux;
            register int i;
        ...
        if(task_to_exec != NULL)
        {
            ...
            switch (task_to_exec->status)
            {
                ...
                case TASK_STATE_DONE:
                    ...
                    if(task_to_exec->id != PPOS_CORE_ID_SYSTEM_TASK_MAIN) free(task_to_exec->context.uc_stack.ss_sp);
    ...
    // Mata as tarefas do sistema 
    for(i = 2; i < PPOS_CORE_ID_SYSTEM_TASK_LENGTH; i++) // A partir da posição 2 inicia as tarefas de sistema que serão escalonadas
    {
        task_switch(&ppos_core_system_tasks[i]);
        free(ppos_core_system_tasks[i].context.uc_stack.ss_sp);
        #ifdef DEBUG_2
        __PPOS_CORE_STD_DEBUG_MSG__("task_finish: tarefa %d desinstalada\n", ppos_core_system_tasks[i].id);
        #endif
    }
...
static task_t *scheduler_in_envelhecimento()
{
    ...
    if( ppos_core_queue_task_ready == NULL)
        return NULL;
    ...
    while(count_tasks++ < ppos_size_tasks_ready)
    {
        ...
        if(task->id != task_selected->id)
        {
            ...
            {
                task_selected->prio_dynamic -= __FATOR_ENVELHECIMENTO__;
            ...
            {
                task->prio_dynamic -= __FATOR_ENVELHECIMENTO__;
...
#undef __FATOR_ENVELHECIMENTO__
#undef __SCHEDULER_IN_FCS__
#undef __SCHEDULER_IN_ENVELHECIMENTO__
#undef __PPOS_SCHEDULER_ALGORITHM__
#undef __PPOS_CHAMAR_IMPLEMENTACAO_SCHEDULER_DIRETAMENTE__
#undef __PPOS_CORE_DEBUG__
#undef __PPOS_DEFAULT_QUANTUM__