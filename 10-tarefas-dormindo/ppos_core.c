...
#define __PPOS_ID_SLEEP_TASK__ 2
...
static task_t ppos_core_task_control_tasks_sleep;
...
static void sleep_tasks(void *arg);
...
void ppos_init ()
{
    ...
    task_init(&ppos_core_task_control_tasks_sleep, &sleep_tasks, NULL);
    ppos_core_task_control_tasks_sleep.is_user_task = false;
...
void task_exit (int exit_code)
{
    ...
    if( ! ppos_core_task_current->is_user_task )
    {
        ...
        // 
        if ( ppos_core_task_current->id == __PPOS_ID_DISPATCHER__ )
            return;
    }
...
{
    // enquanto houver tarefas suspensas
    while (task != NULL)
    {
        ...
        // passa a apontar para a nova cabeca da lista de tarefas suspensas
        task = *queue;
    }
}
...
void task_sleep (int t) 
{
    ...
    //
    //if(queue_append((queue_t **) &ppos_core_queue_task_sleep, (queue_t *) ppos_core_task_current) == 0) ++ppos_size_tasks_sleep;
    ...
    //
    task_append_queue_ready(&ppos_core_task_control_tasks_sleep);
...
static void dispatcher(void * arg)
{
    ...
    // 
    task_remove_queue_ready(&ppos_core_task_control_tasks_sleep);
        ...
        if(task_to_exec != NULL)
        {
            ...
            switch (task_to_exec->status)
            {
                ...
                case TASK_STATE_DONE:
                    ...
                    task_awake(task_to_exec->queue_task_suspend, &task_to_exec->queue_task_suspend);
                break;
    ...
    //
    task_switch(&ppos_core_task_control_tasks_sleep);
    ...
    while ( ppos_size_tasks_sleep )
    {
        ...
        for (i = 0, task_aux = ppos_core_queue_task_sleep; i < ppos_size_tasks_sleep; i++)
        {
            ...
            if (task_aux->time_to_awake <= ppos_core_systime)
            {
                ...
                if (queue_remove((queue_t **) &ppos_core_queue_task_sleep, (queue_t *) task_aux) == 0)  { --ppos_size_tasks_sleep; }

                // adiciona a tarefa da fila de prontos
        ...
        // se nao houver tarefas suspensas, entao tira essa tarefa da fila de prontas do sistena
