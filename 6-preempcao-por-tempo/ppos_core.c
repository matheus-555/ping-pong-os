...
#define __PPOS_DEFAULT_QUANTUM__ 20
...
{  
    ...
    if (setitimer (ITIMER_REAL, &timer, 0) < 0)
    {
        __PPOS_CORE_STD_MSG_ERRO__("Erro ao desarmar o timer: ")
        exit (1) ;
    }
}
...
static void dispatcher(void * arg)
{
    ...
    {
        // Para o timer da contabilizacao do quantum da tarefa, pois essa tarefa eh uma tarefa do sistema operacional
        timer_stop();
        ...
        if(task_to_exec != NULL)
        {
            ...
            if ( task_to_exec->is_user_task )
            {
                ...
                // entao habilita o timer de preempcao da tarefa
                timer_start();
            }
...
static task_t *scheduler_in_envelhecimento()
{
    ...
    while(count_tasks++ < ppos_size_tasks_ready)
    {
        ...
        // Verifica se a task atual possui a mesma prioridade dinamica que a task selecionada
        // se sim, entao va compaar a prioridade estatica
        else if(task->prio_dynamic == task_selected->prio_dynamic)
        {
            // Verifica se a task atual possui mais prioridade estatica que a task escolhida
            // se sim, deixa a task atual executar
            if(task->prio_static <= task_selected->prio_static)
            {
                task_selected = task;
            }
            else
